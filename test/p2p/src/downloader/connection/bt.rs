...
impl Connection for BtConnection {
    ...
    fn receive(&mut self) -> Option<InMessage> {
        match self.receiver.try_recv() {
            ...
            Err(TryRecvError::Disconnected) => Some(InMessage::Error(Error::Closed)),
        }
    }
...
impl Internal {
    fn new(
        ...
        let socket = TcpStream::connect(peer.clone())?;
    ...
    fn decode_raw_message(&self, slice: &[u8]) -> Result<RawMessage, Error> {
        ...
        match slice[0] {
            ...
            5 => Ok(RawMessage::Bitfield(slice[1..].to_vec())),
            6 => {
                if slice.len() == 13 {
    ...
    fn get_raw_message(&mut self) -> Result<Option<RawMessage>, Error> {
        ...
        if self.recv_buffer.len() >= len + 4 {
            let message = {
                ...
                self.decode_raw_message(message_data)?
            };
    ...
    fn write_message(&mut self, msg: RawMessage) -> Result<(), Error> {
        let write_result = match msg {
            RawMessage::KeepAlive => self.write_bytes(&[0, 0, 0, 0]),
            RawMessage::Choke => self.write_bytes(&[0, 0, 0, 1, 0]),
            RawMessage::Unchoke => self.write_bytes(&[0, 0, 0, 1, 1]),
            RawMessage::Interested => self.write_bytes(&[0, 0, 0, 1, 2]),
            RawMessage::NotInterested => self.write_bytes(&[0, 0, 0, 1, 3]),
    ...
    fn check_handshake(&mut self) -> Result<Option<HandshakeInfo>, Error> {
        if self.recv_buffer.len() < 68 {
            ...
            if &self.recv_buffer[0..20] == b"\x13BitTorrent protocol" {
                ...
                for i in 0..20 {
                    hash[i] = self.recv_buffer[28 + i];
                }
                for i in 0..20 {
                    id.0[i] = self.recv_buffer[48 + i];
                }
