...
impl Tracker for HttpTracker {
    ...
    fn peers<'a>(&'a self) -> Box<dyn Iterator<Item = &'a PeerAddress> + 'a> {
...
fn decode_response(value: BValue) -> Result<Response, &'static str> {
    let mut dict = value.get_dict().ok_or("not a dict")?;
...
fn decode_peers(value: BValue) -> Result<Vec<PeerAddress>, &'static str> {
    match value {
        ...
        _ => Err("bad peer list format"),
    }
}

fn decode_peer(value: BValue) -> Result<PeerAddress, &'static str> {
    let mut dict = value.get_dict().ok_or("bad peer")?;
    ...
    let port = dict
        ...
        .and_then(|port| {
            if port >= 0 && port < ::std::u16::MAX as i64 {
                Ok(port as u16)
            } else {
                Err("bad peer port")
            }
