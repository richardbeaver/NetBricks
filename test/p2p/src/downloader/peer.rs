...
use downloader::connection::{Connection, HandshakeInfo, InMessage};
...
impl Peer {
    ...
    pub fn send(&mut self, msg: Message) {
        let msg = match msg {
            Message::Request(piece, off, len) => connection::Message::Request(piece, off, len),
            Message::Piece(piece, off, data) => connection::Message::Piece(piece, off, data),
        };
    ...
    fn store_bitfield(&mut self, bitfield: Vec<u8>) {
        if bitfield.len() != self.have.len() {
            debug!(
                "Peer {:?} sent bad bitfield, length: {}, expected: {}",
                self.peer,
                bitfield.len(),
                self.have.len()
            );
        ...
        for i in 0..spare_bits {
            if last_byte & (1 << i) != 0 {
                debug!(
                    "Peer {:?} sent bad bitfield - some of spare bits are set",
                    self.peer
                );
    ...
    fn process_message(&mut self, msg: connection::Message) -> Option<Message> {
        match msg {
            connection::Message::Choke => self.peer_choked = true,
            connection::Message::Unchoke => self.peer_choked = false,
            connection::Message::Interested => self.peer_interested = true,
            connection::Message::NotInterested => self.peer_interested = false,
            connection::Message::Have(piece) => {
                if piece >= self.piece_count {
                    debug!(
                        "Peer {:?} announced bad piece: {:?}, disconnecting",
                        self.peer, piece
                    );
                    self.connection.close();
                } else {
            ...
            connection::Message::Bitfield(bits) => self.store_bitfield(bits),
