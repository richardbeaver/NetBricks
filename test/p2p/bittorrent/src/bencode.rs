...
impl<'a> Decoder<'a> {
    ...
    fn read_int(&mut self) -> DecodeResult<BValue> {
        ...
        let num = self.read_raw_number('e' as u8)?;
        if negative {
    ...
    fn read_string(&mut self) -> DecodeResult<Vec<u8>> {
        let byte_count = self.read_raw_number(':' as u8)?;
        let bytes = self.read_bytes(byte_count as usize)?.to_vec();
        Ok(bytes)
    }
...
mod test {
    ...
    mod encoding {
        ...
        #[test]
        fn list() {
            check_encode(blist![BValue::Int(42), bstr(b"abc")], b"li42e3:abce");
        }
    ...
    mod decoding {
        ...
        #[test]
        fn list() {
            check_decode(blist![BValue::Int(42), bstr(b"abc")], b"li42e3:abce");
        }
