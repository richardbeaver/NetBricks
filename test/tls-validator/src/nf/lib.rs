                    ...
                    Entry::Occupied(mut e) => {
                        ...
                        if is_client_key_exchange(&p.get_payload()) {
                            ...
                            match payload_cache.entry(rev_flow) {
                                Entry::Occupied(e) => {
                                    ...
                                    match certs {
                                        Ok(chain) => {
                                            ...
                                            if dns_name.is_none() {
                                                println!("DEBUG: DNS name not found, droping" );
                                            } else {
                                                ...
                                                if !result {
                                                    println!("DEBUG: Certificate validation failed, both flows' connection need to be reset\n{:?}\n{:?}\n", flow, rev_flow);
                                ...
                                Entry::Vacant(_) => {
                                    println!("DEBUG: We had a problem: the entry of {:?} doesn't exist", rev_flow)
                                }
                            }

                            match payload_cache.entry(*flow) {
                                Entry::Occupied(_e) =>println!("DEBUG: We had a problem"),
                                Entry::Vacant(_) => println!("Ok"),
                            }
                            match payload_cache.entry(rev_flow) {
                                Entry::Occupied(_e) => println!("DEBUG: We had another problem"),
                                Entry::Vacant(_) => println!("Ok"),
                            }
                        } else {
