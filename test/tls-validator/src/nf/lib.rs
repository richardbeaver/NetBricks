                    ...
                    Entry::Occupied(mut e) => {
                        ...
                        if is_client_key_exchange(&p.get_payload()) {
                            ...
                            match payload_cache.entry(rev_flow) {
                                ...
                                Entry::Vacant(_) => {
                                    info!("We had a problem: the entry of {:?} doesn't exist", rev_flow)
                                }
                            }

                            match payload_cache.entry(*flow) {
                                Entry::Occupied(_e) =>info!("We had a problem"),
                                Entry::Vacant(_) => info!("Ok"),
                            }
                            match payload_cache.entry(rev_flow) {
                                Entry::Occupied(_e) => info!("We had another problem"),
                                Entry::Vacant(_) => info!("Ok"),
                            }
                        } else {
